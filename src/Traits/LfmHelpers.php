...
namespace FeiLongCui\LaravelFileManager\Traits;
...
trait LfmHelpers
{
    ...
    public function getPathPrefix($type)
    {
        ...
        if ($type === 'url' && $base_directory !== 'public') {
            $prefix = config('lfm.url_prefix', config('lfm.prefix', 'laravel-file-manager')) . '/' . $prefix;
        }
    ...
    public function objectPresenter($item)
    {
        ...
        if (! $is_file) {
            $file_type = trans('laravel-file-manager::lfm.type-folder');
            $icon = 'fa-folder-o';
            $thumb_url = asset('vendor/laravel-file-manager/img/folder.png');
    ...
    {
        return trans('laravel-file-manager::lfm.error-' . $error_type, $variables);
    }
